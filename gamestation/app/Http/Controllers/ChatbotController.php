    ...
    public function sendMessage(Request $request)
    {
        ...
        if ($apiKey) {
            try {
                ...
                $products = Product::with(['publisher', 'category'])->where('is_active', true)->get();
...
Đặc biệt, khi khách hàng nhờ tư vấn hoặc đề xuất sản phẩm theo nhu cầu (ví dụ: tìm game theo thể loại, hệ máy, mức giá, sở thích hoặc số người chơi...), hãy phân tích kỹ danh sách sản phẩm ở trên để chọn lọc, tư vấn và gợi ý các sản phẩm phù hợp nhất kèm theo đường dẫn markdown của từng game.";
    ...
    public function getProactiveMessage(\Illuminate\Http\Request $request)
    {
        $user = $request->user();
        if (!$user) {
        ...
        if (!$apiKey) {
            return response()->json([
                'success' => true,
                'message' => 'Chào bạn! Shop có thể hỗ trợ tư vấn sản phẩm hoặc tra cứu đơn hàng giúp bạn ngay lúc này được không ạ?'
            ]);
        }
        ...
        if ($currentUrl) {
            ...
            $path = $parsedUrl['path'] ?? '';
            if (preg_match('/\/products\/([a-z0-9\-]+)/i', $path, $matches)) {
        ...
        $targetProduct = null;
        if ($productSlug) {
            $targetProduct = Product::where('slug', $productSlug)->where('is_active', true)->first();
        }

        $cart = \App\Models\Cart::with('items.product')->where('user_id', $user->id)->first();

        // Build prompt instruction for proactive greeting
        $prompt = "Bạn là trợ lý ảo AI của GameStation. Hãy soạn một câu chào hỏi và gợi ý chủ động ngắn gọn (dưới 35 từ, không dài dòng, không dùng ký hiệu lạ) bằng tiếng Việt gửi tới khách hàng. ";
        
        if ($targetProduct) {
            $prompt .= "Khách hàng hiện đang xem trang chi tiết của game: '{$targetProduct->name}' (Hệ máy: " . strtoupper($targetProduct->platform ?? 'N/A') . ", Giá: " . number_format($targetProduct->price, 0, ',', '.') . "đ). Hãy đưa ra một lời chào thân thiện nhắc đến tựa game này và hỏi xem họ có cần tư vấn gì về lối chơi hay khuyến mãi của game này không.";
        } elseif ($cart && $cart->items->isNotEmpty()) {
            $cartItemsStr = $cart->items->map(fn($item) => $item->product ? $item->product->name : '')->filter()->implode(', ');
            $prompt .= "Khách hàng đang có sản phẩm trong giỏ hàng: [{$cartItemsStr}]. Hãy soạn một câu chào nhắc khéo họ có sản phẩm đang chờ thanh toán trong giỏ hàng và hỏi xem họ có cần hỗ trợ tư vấn gì thêm để hoàn tất đặt hàng không.";
        } else {
            $prompt .= "Khách hàng đang lướt xem website. Hãy soạn một câu chào mừng thân thiện giới thiệu rằng shop đang có nhiều đĩa game PS5, Nintendo Switch cực hot mới về và hỏi xem họ cần tìm thể loại game gì để shop tư vấn.";
        }
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if ($reply) {
                    $replyText = trim(str_replace(['"', "'"], '', $reply));
                    
                    // Save to database as a bot message so history stays synchronized
                    $botMessage = ChatbotMessage::create([
                        'user_id' => $user->id,
                        'sender' => 'bot',
                        'message' => $replyText,
            ...
            \Illuminate\Support\Facades\Log::error("Gemini Proactive Error: " . $e->getMessage());
        }

        // Fallback
        $fallback = 'Chào bạn! Shop có thể hỗ trợ tư vấn sản phẩm hoặc giải đáp thắc mắc về đơn hàng của bạn ngay bây giờ được không ạ?';
        $botMessage = ChatbotMessage::create([
            'user_id' => $user->id,
            'sender' => 'bot',
            'message' => $fallback,
